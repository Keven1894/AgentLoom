# User Manual: Building Your AI Agent with AgentLoom
...
This manual guides **YOU** (the project owner) through using the `agentLoom` package to build a custom AI agent for your domain.
...
3. **Copy protocol package**:
   - Copy entire `agentLoom/` folder into `my-agent-project/`
...
```
# Task: Build Custom AI Agent Using AgentLoom

Read the protocol package:
@agentLoom/phases/INDEX.md
...
Follow the AI Agent Setup AgentLoom (9 phases):
...
To ensure optimal AI performance, AgentLoom enforces strict document size limits.
...
```
Check my current progress in the AgentLoom build:
...
```
Resume AgentLoom build from Phase [X]:

1. Read agentLoom/outputs/phase-0[X-1].md (previous phase)
2. Read agentLoom/phases/PHASE_0[X]_*.md (current phase instructions)
...
```
Validate my current AgentLoom build state:

1. Run: python agentLoom/builder-assets/scripts/validate_structure.py
2. If graphs exist, run: python agentLoom/builder-assets/scripts/validate_graphs.py
...
@agentLoom/phases/PHASE_0[X]_*.md
...
```
Check if agentLoom/outputs/phase-0[X].md exists.

If missing: Regenerate it following the spec at agentLoom/specs/phase-0[X]-output.spec.md
...
1. python agentLoom/builder-assets/scripts/validate_structure.py
...
Read: agentLoom/outputs/phase-02.md
...
1. Read the spec: agentLoom/specs/[spec-name].spec.md

# Contributors

This project exists thanks to all the people who contribute to advancing production-grade agentic AI engineering.

---

## 🎯 Core Team

### Framework Creator & Lead Developer

**Dr. Boyuan (Keven) Guan**

- **Affiliation:** Florida International University, Library & GIS Center
- **Role:** Framework creator, lead developer, research director
- **Email:** <bguan@fiu.edu>
- **Contributions:**
  - Agentic-AI Engineering Framework design
  - Dual-helix architecture (Engineering + Governance)
  - DIVA case study (production validation)
  - Research roadmap and methodology
  - Framework foundations and theoretical grounding
  - Memory hierarchy analogy and adaptive framework concept
- **Publications:**
  - Guan, B. (2025). AgentLoom: Building Trustworthy AI Agents Through Dual-Helix Architecture. DOI: 10.5281/zenodo.17561541

---

## 📚 Case Study Contributors

### Digital Libraries & Institutional Repositories

#### DIVA: Dataverse Intelligent Virtual Assistant

**Authors:** Dr. Boyuan (Keven) Guan  
**Affiliation:** FIU Library & GIS Center  
**Case Study:** [Dataverse DIVA](docs/case-studies/dataverse-diva.md)  
**Framework Stage:** Stage 2 (RAM: Indexing + Metadata + Docs)  
**Status:** Production (3+ months)  
**Novel Contributions:**

- Rules-as-memory institutional memory system (100% consistency, 87% token efficiency)
- Schema-based context engineering (+23% accuracy for Llama 3.2 3B)
- Multi-role single agent architecture (SysAdmin + Developer)
- Tiered configuration system for memory hierarchy
- First AI agent specifically for digital library systems

**Metrics:**

- LLM accuracy: 52.6% (Llama 3.2 3B on complex document comprehension)
- Deployment success: 100% (50+ deployments)
- Team satisfaction: 9.2/10
- Documentation: 157+ files, 10,000+ lines
- Time savings: 60-100 hours/month

---

### [Your Domain Here]

**Awaiting contributions!**

We're seeking case studies in:

- GIS / Environmental Data
- Education / Research Support
- IT Operations / DevOps
- Healthcare / Clinical Systems
- Industry Applications
- Other innovative domains

**Want to contribute?** See [Case Studies README](docs/case-studies/README.md)

---

## 💻 Code Contributors

### Framework Implementation

[Auto-generated from git log - to be implemented]

**Current:** Core framework by Dr. Guan

**Future:** Community contributions to:

- Context capture utilities
- Documentation generation tools
- Indexing frameworks
- RAG pipeline implementations
- Fine-tuning scripts
- Evaluation benchmarks

---

## 📖 Documentation Contributors

### Framework Documentation

**Primary Author:** Dr. Boyuan (Keven) Guan

**Core Documents:**

- README.md — Project overview
- docs/Agentic-AI-Research-Roadmap.md — Complete methodology
- docs/framework-foundations.md — Theoretical underpinnings
- docs/research-problems-and-positioning.md — Academic positioning
- docs/dual-helix-clarification.md — Engineering vs. Governance
- docs/Research-Timeline-2025-2027.md — Roadmap and milestones

### Community Documentation

[Awaiting community contributions]

**Opportunities:**

- Translations to other languages
- Domain-specific guides
- Tutorial development
- Video content creation

---

## 🎨 Design & Visualization Contributors

### Diagrams and Figures

**Specifications by:** Dr. Boyuan (Keven) Guan

**Current Specifications:**

- figures/dual-helix-diagram-spec.md
- figures/memory-hierarchy-analogy-diagram.md

**Seeking:**

- Graphic designers to render diagram specifications
- Data visualization experts for metrics charts
- UX designers for documentation site

**Contact:** <bguan@fiu.edu> with "Design Contribution" in subject

---

## 🔬 Research Collaborators

### Academic Partners

**Current:**

- Florida International University — Library & GIS Center (lead)
- FIU Computer Science Department
- FIU Division of IT

**Seeking:**

- Academic institutions for domain validation
- Research labs for theoretical extensions
- Industry partners for production pilots

**Collaboration Types:**

- Multi-domain validation studies
- Formal verification research
- Benchmark development
- Standards creation

**Contact:** <bguan@fiu.edu> with "Research Partnership" in subject

---

## 🌟 Special Recognition

### Major Contributions

**Significant contributions will be recognized through:**

- Co-authorship on academic papers
- Invited talks at conferences
- Featured in project communications
- Leadership roles in community initiatives

### Community Champions

**Recognition for:**

- Sustained engagement and support
- Helping others in discussions
- Spreading awareness of the project
- Organizing community events

[None yet — be the first!]

---

## 🎓 Acknowledgments

### Institutions

**Florida International University:**

- Library & GIS Center (lead institution)
- Knight Foundation School of Computing and Information Sciences
- Division of IT
- Digital Collections Center

### Research Infrastructure

- Pelican Data Platform
- Open Science Grid (OSG)
- Harvard IQSS (Dataverse project)

### Inspiration & Foundations

This work builds upon:

- Computer architecture principles (Hennessy & Patterson)
- Knowledge management theory (Nonaka & Takeuchi)
- Software engineering best practices
- LLM and agentic AI research community

---

## 📊 Contribution Statistics

### Current Status (as of November 11, 2025)

- **Case Studies:** 1 (DIVA — Digital Libraries)
- **Core Documents:** 10+ framework documents
- **Research Papers:** In preparation (4 planned)
- **Community Members:** Growing
- **Commits:** [Auto-count from git]
- **Contributors:** [Auto-count from git]

### Target for 2026

- **Case Studies:** 5-8 across different domains
- **Research Papers:** 4-6 published
- **Community:** 20+ active contributors
- **Implementations:** Multiple production deployments

---

## 🚀 How to Be Listed Here

### As Case Study Contributor

1. Apply framework to your project
2. Submit case study via PR
3. Merge approval → automatic listing

**Full process:** [Case Studies README](docs/case-studies/README.md)

### As Core Team Member

- By invitation for sustained framework development
- Contact: <bguan@fiu.edu>

### As Code Contributor

- Submit code/tool contributions via PR
- Automatic listing from git commit history

### As Community Champion

- Active engagement in discussions
- Helping others
- Spreading awareness
- Recognition by core team

---

## 💡 Contribution Ideas

**Not sure where to start?**

### For Researchers

- [ ] Apply framework to your research domain
- [ ] Validate framework stages in your project
- [ ] Develop domain-specific patterns
- [ ] Contribute empirical studies

### For Engineers

- [ ] Build framework implementation tools
- [ ] Create automation utilities
- [ ] Develop RAG pipeline components
- [ ] Contribute evaluation scripts

### For Educators

- [ ] Develop course modules
- [ ] Create tutorials and guides
- [ ] Design hands-on exercises
- [ ] Contribute teaching case studies

### For Practitioners

- [ ] Share your production deployment experiences
- [ ] Document lessons learned
- [ ] Contribute best practices
- [ ] Help others in discussions

---

## 📞 Contact

**Project Lead:**  
Dr. Boyuan (Keven) Guan  
<bguan@fiu.edu>  
FIU Library & GIS Center

**Project Website:**  
<https://dataversedev.fiu.edu/ai/>

**Repository:**  
<https://github.com/Keven1894/AgentLoom>

---

## 🙏 Thank You

To everyone who contributes—whether through case studies, code, documentation, research collaboration, or community support—**thank you!**

Your work helps establish the engineering discipline for the next generation of intelligent systems.

**Together, we're building reliable agentic AI.** 🤖💙

---

**Last Updated:** November 11, 2025  
**Status:** Living document (updates with each contribution)  
**Maintained by:** Core team

**Note:** This file will be automatically updated as contributions are merged. Git commit history provides the authoritative record of all contributors.
